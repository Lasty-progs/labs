...
class Schedule(QWidget):
    ...
    def _update_days_tables(self):
        for day in range(len(self.days)):
            self.cursor.execute(f"SELECT tt.subject, te.full_name, tt.room_numb, tt.start_time, tt.id\
                                FROM timetable AS tt JOIN teacher AS te ON te.subject=tt.subject\
                                WHERE day=%s and week=%s ORDER BY tt.start_time", [str(self.days[day]), str(self.week)])
    ...
    def _change_day_from_table(self, rowNum, day, id):
        ...
        try:
            if row.count('') == self.days_tables[day].columnCount() - 1:
                ...
                self.cursor.execute(f"UPDATE timetable SET subject='{row[0]}', start_time='{row[3]}', room_numb={row[2]}\
                                    WHERE id={id}")
            self.conn.commit()
