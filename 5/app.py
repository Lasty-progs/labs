    ...
    if request.method == 'POST':
        ...
        if record:
            ...
            if (login and password and name) and (login.count(' ') == 0 and password.count(' ') == 0
                and name.count(' ') == 1 and name.find(' ') != 0 and name.find(' ') != (len(name)-1)):
                
                cursor.execute('INSERT INTO service.users (full_name, login, password) VALUES (\'{0}\', \'{1}\', \'{2}\');'.format(str(name), str(login), str(password)))
